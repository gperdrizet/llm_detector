...
def length_distributions(title: str, data_df: pd.DataFrame, rows: int=3, cols: int=2, hue_by: bool=None) -> plt:
    ...
    for i, dataset in enumerate(datasets.index):
        ...
        dist=sns.kdeplot(data=plot_data_df, x='words', hue=hue_by, log_scale=10, legend=True, ax=axs[i])
    ...
    dist=sns.kdeplot(data=data_df, x='words', hue=hue_by, log_scale=10, legend=True, ax=axs[-1])
    if hue_by is not None:
        dist.get_legend().set_title(None)
...
def bin_density_scatter(title: str, data_df: pd.DataFrame, rows: int=3, cols: int=2, num_bins: int=100) -> plt:
    ...
    for i, dataset in enumerate(datasets.index):
        ...
        human_density, human_bins=np.histogram(plot_data_df['words'][plot_data_df['synthetic'] == 'human'], bins=bins, density=True)
        synthetic_density, synthetic_bins=np.histogram(plot_data_df['words'][plot_data_df['synthetic'] == 'synthetic'], bins=bins, density=True)
    ...
    counts, bins=np.histogram(data_df['words'], bins=num_bins)
    human_density, human_bins=np.histogram(data_df['words'][data_df['synthetic'] == 'human'], bins=bins, density=True)
    synthetic_density, synthetic_bins=np.histogram(data_df['words'][data_df['synthetic'] == 'synthetic'], bins=bins, density=True)
